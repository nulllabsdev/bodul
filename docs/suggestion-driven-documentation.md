...
copy and promotes the proposal to the target file (Applying). The human
reviewer's authority is exercised during Reviewing, before applying begins; the
applier does not decide which suggestions to implement.
...
1. If `docs/_resources/{document-name}/` or its `suggestions/` folder does not
   exist, create it. If `docs/_resources/{document-name}/INDEX.md` does not
   exist, create it using the suggestion-driven schema (see
   [INDEX.md contents](#indexmd-contents)) before review begins.
2. Review the target file and write suggestions into
   `docs/_resources/{document-name}/suggestions/`.
3. Commit the suggestion documents.
...
6. Once every accepted suggestion targeting a proposal has been resolved,
   copies the proposal over the target file at its original path. The
   `{document-name}` key is lossy and cannot be reversed, so the applier reads
   the original path from the `File` field of the suggestion documents. Then
   updates `docs/_resources/{document-name}/INDEX.md`, and sets `Decision` to
