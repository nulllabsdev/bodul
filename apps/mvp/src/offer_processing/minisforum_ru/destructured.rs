...
mod tests {
    ...
    fn availability_round_trips() {
        for (url, variant) in [
            (
                ...
                MinisForumRuAvailability::InStock,
            ),
            (
                "http://schema.org/OutOfStock",
                MinisForumRuAvailability::OutOfStock,
            ),
        ] {
            let from_url: MinisForumRuAvailability =
    ...
    fn deserializes_every_ru_page() {
        ...
        for entry in std::fs::read_dir(dir)
            .expect("RU destructed dir exists")
            .flatten()
        {
            let path = entry.path();
            ...
            let _product: MinisForumRuDestructuredProduct = serde_json::from_str(&raw)
