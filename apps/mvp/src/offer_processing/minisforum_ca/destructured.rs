...
//! `destructure` binary writes to `data/pages-destructed/MinisForumCa/`: every
...
mod tests {
    ...
    fn availability_round_trips() {
        for (url, variant) in [
            (
                ...
                MinisForumCaAvailability::InStock,
            ),
            (
                "https://schema.org/OutOfStock",
                MinisForumCaAvailability::OutOfStock,
            ),
        ] {
            let from_url: MinisForumCaAvailability =
    ...
    fn deserializes_every_ca_page() {
        let dir = std::path::Path::new("data/pages-destructed/MinisForumCa");
        let mut count = 0;
        for entry in std::fs::read_dir(dir)
            .expect("CA destructed dir exists")
            .flatten()
        {
            let path = entry.path();
            ...
            let _product: MinisForumCaDestructuredProduct = serde_json::from_str(&raw)
