...
//! `destructure` binary writes to `data/pages-destructed/MinisForumAu/`: every
...
mod tests {
    ...
    fn availability_round_trips() {
        for (url, variant) in [
            (
                ...
                MinisForumAuAvailability::InStock,
            ),
            (
                "https://schema.org/OutOfStock",
                MinisForumAuAvailability::OutOfStock,
            ),
        ] {
            let from_url: MinisForumAuAvailability =
    ...
    fn deserializes_every_au_page() {
        let dir = std::path::Path::new("data/pages-destructed/MinisForumAu");
        let mut count = 0;
        for entry in std::fs::read_dir(dir)
            .expect("AU destructed dir exists")
            .flatten()
        {
            let path = entry.path();
            ...
            let _product: MinisForumAuDestructuredProduct = serde_json::from_str(&raw)
