...
//! `destructure` binary writes to `data/pages-destructed/MinisForumFr/`: every
...
mod tests {
    ...
    fn availability_round_trips() {
        for (url, variant) in [
            (
                ...
                MinisForumFrAvailability::InStock,
            ),
            (
                "http://schema.org/OutOfStock",
                MinisForumFrAvailability::OutOfStock,
            ),
        ] {
            let from_url: MinisForumFrAvailability =
    ...
    fn deserializes_every_fr_page() {
            ...
            .flatten()
        {
            ...
            let _product: MinisForumFrDestructuredProduct = serde_json::from_str(&raw)
