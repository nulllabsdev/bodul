...
//! `destructure` binary writes to `data/pages-destructed/MinisForumEu/`: every
...
mod tests {
    ...
    fn availability_round_trips() {
        for (url, variant) in [
            (
                ...
                MinisForumEuAvailability::InStock,
            ),
            (
                "http://schema.org/OutOfStock",
                MinisForumEuAvailability::OutOfStock,
            ),
        ] {
            let from_url: MinisForumEuAvailability =
    ...
    fn deserializes_every_eu_page() {
        let dir = std::path::Path::new("data/pages-destructed/MinisForumEu");
        let mut count = 0;
        for entry in std::fs::read_dir(dir)
            .expect("EU destructed dir exists")
            .flatten()
        {
            let path = entry.path();
            ...
            let _product: MinisForumEuDestructuredProduct = serde_json::from_str(&raw)
