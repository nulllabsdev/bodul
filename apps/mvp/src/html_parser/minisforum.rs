...
//!
//! [`architecture`] assembles the common structures, gated by the config. Blocks
//! that may be absent on a given store (e.g. the `feature_chart` /
//! `main_product` segments) are always included: a segment whose selector
//! matches nothing simply yields no output, so it is harmless.

use super::structure::{RetailerArchitecture, Structure, collection};
...
pub fn architecture(config: Config) -> RetailerArchitecture {
