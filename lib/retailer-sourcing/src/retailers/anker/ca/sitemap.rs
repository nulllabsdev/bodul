...
pub fn sitemap_config() -> SitemapConfig {
    SitemapConfig {
        sitemap_url: vec!["https://www.anker.com/ca/sitemap.xml".to_string()],
    }
}

pub fn classify_link(url: &str, source: &str, _image_count: usize) -> LinkKind {
    if let Some(kind) = classify_by_source(source) {
        if !url.to_lowercase().starts_with("https://www.anker.com/ca/") {
...
fn classify_by_source(source: &str) -> Option<LinkKind> {
    match source {
        "https://www.anker.com/ca/sitemap-0.xml" => Some(LinkKind::NotInterested),
        "https://www.anker.com/ca/server-sitemap-index-pages.xml" => Some(LinkKind::NotInterested),
        "https://www.anker.com/ca/server-sitemap-index-products.xml" => Some(LinkKind::Product),
        "https://www.anker.com/ca/server-sitemap-index-collections.xml" => Some(LinkKind::Catalog),
        "https://www.anker.com/ca/server-sitemap-index-blog.xml" => Some(LinkKind::Content),
        _ => None,
    }
}
