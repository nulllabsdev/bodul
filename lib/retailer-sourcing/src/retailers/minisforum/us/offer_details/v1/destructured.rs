...
//! `destructure` binary writes to `data/offers-destructed/MinisForumUs/`: every
...
mod tests {
    ...
    fn availability_round_trips() {
        for (url, variant) in [
            ("http://schema.org/InStock", MinisForumUsAvailability::InStock),
            ("http://schema.org/OutOfStock", MinisForumUsAvailability::OutOfStock),
        ] {
            let from_url: MinisForumUsAvailability = serde_json::from_str(&format!("\"{url}\"")).unwrap();
    ...
    fn deserializes_every_us_page() {
        let dir = std::path::Path::new("data/offers-destructed/MinisForumUs");
        let mut count = 0;
        for entry in std::fs::read_dir(dir).expect("US destructed dir exists").flatten() {
            let path = entry.path();
            if path.extension().is_none_or(|ext| ext != "json") {
                continue;
            }
            let raw = std::fs::read_to_string(&path).expect("reads file");
            let _product: MinisForumUsDestructuredProduct =
