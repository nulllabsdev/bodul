...
//! the JSON the `destructure` binary writes to `data/offers-destructed/MinisForumHk/`:
...
mod tests {
    ...
    fn availability_round_trips() {
        for (url, variant) in [
            ("https://schema.org/InStock", MinisForumHkAvailability::InStock),
            ("https://schema.org/OutOfStock", MinisForumHkAvailability::OutOfStock),
        ] {
            let from_url: MinisForumHkAvailability = serde_json::from_str(&format!("\"{url}\"")).unwrap();
    ...
    fn deserializes_every_hk_page() {
        let dir = std::path::Path::new("data/offers-destructed/MinisForumHk");
        let mut count = 0;
        for entry in std::fs::read_dir(dir).expect("HK destructed dir exists").flatten() {
            let path = entry.path();
            if path.extension().is_none_or(|ext| ext != "json") {
                continue;
            }
            let raw = std::fs::read_to_string(&path).expect("reads file");
            let _product: MinisForumHkDestructuredProduct =
