//! Page architecture for UGREEN NAS UK (United Kingdom storefront).
//!
//! Locale port of `ugreen_nas` — identical Shopify "t4s" theme; see that
//! module for the source-by-source rationale. Store-specific tags absent on
//! some regions (e.g. `omega:product`, `#glood-loader`) simply no-op here.

use crate::parsing::structure::RetailerArchitecture;
use crate::parsing::structure::{collection, json, json_after, particle, scrub, segment};
